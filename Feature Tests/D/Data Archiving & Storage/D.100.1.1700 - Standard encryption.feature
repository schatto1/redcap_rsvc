Feature: Data Archiving / Storage: Standard encryption will be employed for the EIC files. Will be employed in transit and at rest.
  ...
  Scenario: D.100.1.1700.100 Backup storage location encryption
    ...
    Then I should see "Archive Solution at the Center For High Performance Computing"
  ...
  Scenario: D.100.1.1700.200 Transmission traffic encryption
    ...
    Then I should see "Transport Encryption"
    And I should see "Bareos uses TLS (Transport Layer Security) to provide secure network transport. For data encryption in contrast, please see the Data Encryption chapter. The initial Bacula encryption implementation has been written by Landon Fuller."
    And I should see "With Version >= 18.2.4 the TLS code has been enhanced by the TLS-PSK (Pre Shared Keys) feature which allows the daemons to setup an encrypted connection directly without using certificates. The library used for TLS is openSSL."
